...
def login_user():
    ...
    try:
        ...
        if(user.verify(data['password'])):
            token = jwt.encode({'userId': user.id, 'exp': datetime.utcnow(
...
def get_deposit(caller):
    ...
    try:
        ...
        deposit = db.session.query(Deposit).join(
...
def transfer(caller):
    try:
        ...
        if (caller.userType == 'client' and caller.id != sender_deposit.userId):
            return jsonify({'message': 'Unauthorized call'}), 400
...
def get_incoming_transfers(caller):
    ...
    try:
        ...
        transfers = db.session.query(Transfer).join(Deposit, and_(
            Transfer.receiver == Deposit.id)).join(User).filter(User.id == userId)
...
def get_outgoing_transfers(caller):
    ...
    try:
        ...
        transfers = db.session.query(Transfer).join(Deposit, and_(
            Transfer.sender == Deposit.id)).join(User).filter(User.id == userId)
...
def get_loans(caller):
    ...
    try:
        ...
        loans = db.session.query(Loan).join(Deposit).join(
            User).filter(User.id == userId).all()
...
def create_loan(caller):
    try:
        ...
        loan = Loan(deposit.id, data['value'],
...
def filter_loans(caller):
    try:
        ...
        if('status' and 'iban' in data):
            loans = db.session.query(Loan).join(Deposit).filter(
                Deposit.id == data['iban'] and Loan.status == data['status']).all()
        elif('status' in data):
            loans = db.session.query(Loan).filter(
                Loan.status == data['status']).all()
        elif('iban' in data):
            loans = db.session.query(Loan).join(Deposit).filter(
                Deposit.id == data['iban']).all()
...
def change_loan_status(caller):
    try:
        ...
        if (caller.userType != 'admin'):
            return jsonify({'message': 'Unauthorized call'}), 401

        loan = db.session.query(Loan).filter(Loan.id == data['id']).first()
        print(loan)
        ...
        if(data['status'] == 'open' and loan.status == "pending"):
            ...
            liability = Account.query.filter_by(
                description="liability").first()
